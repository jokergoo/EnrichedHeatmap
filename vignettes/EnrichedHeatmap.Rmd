...
Similar as the normal heatmap, colors are controlled by a vector or a color mapping function.

```{r, fig.width = 3}
EnrichedHeatmap(mat1, col = c("white", "red"), name = "H3K4me3")
library(circlize)
EnrichedHeatmap(mat1, col = colorRamp2(quantile(mat1, seq(0, 0.95, length = 10)), cm.colors(10)), 
    name = "H3K4me3")
```
...
There is a special column annotation function anno_enriched() which shows mean values of columns (i.e. mean signals) in the normalized matrix.
...
The values for these windows which contain no CpG sites can be imputed by smoothing.

...
you need to manually set the color mapping if you want to make figures comparable.
...
Target regions are also splitted into windows, but since width of the target regions are different from each other, 
...
If you are interested in a small cluster which is maybe located close to TSS, under the interactive mode,
you can use mouse to select this region by `select()` function, and it will give you the order of rows
for these selected sub-region.
...
huge amout of memory if your `GRanges` object contains too many regions. `s` argument in normalizeToMatrix()`

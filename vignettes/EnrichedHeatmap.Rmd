...
visualize e.g. how histone marks are enriched to specific sites.

There are several tools that can make such heatmap (e.g. [ngs.plot](https://github.com/shenlab-sinai/ngsplot).
Here we implement Enriched heatmap by **ComplexHeatmap** package. Since this type of heatmap is just a
normal heatmap but with some fixed settings, with the functionality of **ComplexHeatmap**, it would be much easier
to customize the heatmap as well as concatenating a list of heatmaps to show correspondance between differnet data sources.
...
`normalizeToMatrix()` converts the association between genomic signals (`H3K4me3`) and tarets(`tss`) in to a matrix. 
It first splits the extended targets regions ( extend to upstream and downstream controlled by `extend` argumetn)
into a list of small windows (controlled by `w`), then overlaps genomic signals to these small windows and calculates
...
There is a special column annotation function which shows mean values of columns in 
...
you need to manully set the color mapping if you want to make figures comparable.

In following example, we use `colorRamp2()` from **circlize** package to define
a color mapping function with break values.
...
Some graphical settings specific for the `EnrichedHeatmap` object:
...
setting `mean_mode` to `absolute` seems to reasonable. For other case, `mean_mode` can be set to
`weighted` or `w0`.

The target of the enrichment can not only be a single point but also can be region with width larger than 1.
...
Thanks for the functionality of **ComplexHeatmap** package, heatmaps can be concatenated
by `+` operator. `EnrichedHeatmap` and `Heatmap` can be mixed.
...
To illustrate more clearly, we load the example data. 
...
In this example, `neg_cr` contains regions that shows negative correlation between methylation
...
2. for every gene, use a sliding window to go through from left to right
    and find correlated regions by looking at the correlation for every gene

Since genes may be close to each other, it may be the case that one corrlated region for gene A
overlaps with gene B. This is not what we want and by specifying the mapping, we can correspond 
correlated regions to the right genes.
...
EnrichedHeatmap(mat4, col = c("white", "green"), name = "neg_cr",
    top_annotation = HeatmapAnnotation(lines = anno_enriched(mat4, gp = gpar(col = "green"))), 
    top_annotation_height = unit(2, "cm"))
```
...
actually enriched at the TSS of transcripts.
...
ht_list = EnrichedHeatmap(mat4, col = c("white", "green"), name = "neg_cr",
              top_annotation = HeatmapAnnotation(lines1 = anno_enriched(mat4, gp = gpar(col = "green"))), 
              top_annotation_height = unit(2, "cm")) +
          EnrichedHeatmap(mat5, col = c("white", "black"), name = "tx",
              top_annotation = HeatmapAnnotation(lines2 = anno_enriched(mat5, gp = gpar(col = "black"))), 
              top_annotation_height = unit(2, "cm"))
...
Due to `findOverlaps()` in **GenomicRanges** package, `normalizeToMatrix()` may eat 
huge amout of memory if your `GRanges` object contains too many regions. 

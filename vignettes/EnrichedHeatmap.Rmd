...
Please note for **w0** mode, the sum is always divided by the width of the window, which means we assume
the genomic signals do not overlap to each other. Actually there is some solution to deal with overlapping
signals with **w0** mode but the implementation is too slow and it is not supported yet now. However, users
can still do it by modifying the signals a little bit and change to **weighted** mode. 

```{r, eval = FALSE}
signal_complement = gaps(signals)
# if there is no strand info for signals
signal_complement = signal_complement[strand(signal_complement) == "*"]
signal_complement[[value_column]] = 0
signal = c(signal, signal_complement)
normalizeToMatrix(signal, mean_mode = "weighted")
```

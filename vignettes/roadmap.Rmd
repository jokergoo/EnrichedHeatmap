...
Each row in the normalized matrix is a single gene, the correlation between histone modification and gene expression can
...
to order genes is more important to reveal patterns for all heatmaps.

To method to order genes depends on what patterns you want to reveal. In this analysis, the message we want to show
...
row clusters, in order to make the plot more clear, "cluster2|high" is removed from following analysis.
...
Add the heatmap which shows enrichment of CGI to TSS. Top annotation which show enrichment pattern is added
...
For each heatmap list, a single column heatmap (or bar) is attached to the most right side to represent difference

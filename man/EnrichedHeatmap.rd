...
\details{
\code{EnrichedHeatmap-class} is inherited from \code{Heatmap-class}. Following parameters are  set with pre-defined values:  
...
With above pre-defined values, no graphics will be drawn below the heatmap, then the space below the heatmap is used to add a new graph which contains the axis. A (or two) line which corresponds to  the position of \code{target} will be added to the heatmap body as well.  

Same as the \code{\link[ComplexHeatmap]{Heatmap-class}}, users can make controls on the heatmap such as apply clustering on rows (this will ignore the \code{row_order}), split rows by data frame or k-means clustering.  
...
\section{Example}{
load(paste0(system.file("/extdata/chr21_test_data.RData",      package = "EnrichedHeatmap"))) mat3 = normalizeToMatrix(meth, cgi, value_column = "meth", mean_mode = "absolute",     extend = 5000, w = 50, empty_value = 0.5) EnrichedHeatmap(mat3, name = "methylation", column_title = "methylation near CGI")  

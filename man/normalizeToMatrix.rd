...
\arguments{
  ...
  \item{value_column}{index for column in \code{gr} that will be mapped to colors. If it is \code{NULL}, an internal column which contains 1 will be attached.}
  ...
  \item{smooth}{whether apply smoothing in every row in the matrix. The smoothing is applied by \code{\link[stats]{loess}}. Please note the data range will change, you need to adjust values in the new matrix afterwards.}
  \item{span}{degree of smoothing, pass to \code{\link[stats]{loess}}.}
  \item{s}{\code{\link[GenomicRanges]{findOverlaps}} sometimes uses a lot of memory. \code{target} is splitted into \code{s} parts and each part is processed serialized.}
  \item{trim}{percent of extreme values to remove}

}
\details{
In order to visualize associations between \code{gr} and \code{target}, the data is transformed into a matrix and visualized as a heatmap.  
...
\section{Example}{
gr = GRanges(seqnames = "chr1",  	  ranges = IRanges(start = c(1, 4, 7, 11, 14, 17, 21, 24, 27),                      end = c(2, 5, 8, 12, 15, 18, 22, 25, 28)),     score = c(1, 2, 3, 1, 2, 3, 1, 2, 3)) target = GRanges(seqnames = "chr1", ranges = IRanges(start = 10, end = 20)) normalizeToMatrix(gr, target, extend = 10, w = 2) normalizeToMatrix(gr, target, extend = 10, w = 2, include_target = TRUE) normalizeToMatrix(gr, target, extend = 10, w = 2, value_column = "score")  
